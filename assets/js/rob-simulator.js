...
if (root) {
  ...
  const levels = [
    ...
    { name: 'Key Workshop', floor: 0x241b32, grid: 0xc548e8, gate: [-7.8, 1.7], dock: [7.7, -5.7], key: KEY_WORKSHOP_KEY_SPAWN, door: [2.8, 0, .5, 5], cells: [[-1.3, -2.3], [7.2, 1.5], [4.5, -4.8]], enemies: [['spider', -4.8, 3.8], ['dalek', 6, -4], ['spider', -1, -4.6]], health: 2, speed: .18, bonus: 1100, obstacles: [[-5.5, -.5, 1.1, 5.8, 1.5], [-.4, 2.1, 5.2, 1, 1.1]] },
  ...
  const buildROB = () => {
    ...
    const baseFlipper = new THREE.Group(); baseFlipper.name = 'Base Lift Flipper Assembly'; baseFlipper.position.set(0, .52, -.62); r.add(baseFlipper); robotRig.baseFlipper = baseFlipper;
    const flipperMotor = mesh(new THREE.CylinderGeometry(.16, .16, .68, 18), hub, baseFlipper, 0, 0, 0); flipperMotor.name = 'Base Lift Flipper Motor'; flipperMotor.rotation.z = Math.PI / 2;
    const flipperBlade = mesh(new THREE.BoxGeometry(.92, .12, 1.28), steel, baseFlipper, 0, -.04, -.66); flipperBlade.name = 'Base Lift Flipper Blade';
    const flipperFoot = mesh(new THREE.BoxGeometry(1.08, .16, .28), mat(0xff8b2f, 0x652504), baseFlipper, 0, -.04, -1.32); flipperFoot.name = 'Base Lift Flipper Floor Pad';
  ...
  const loadLevel = (index) => {
    ...
    robot.position.set(0, 0, ARENA_HALF_DEPTH - 1.6); robot.rotation.set(0, 0, 0); robotRig.baseFlipper.rotation.set(0, 0, 0); robotRig.torso.rotation.set(0, 0, 0); armAssemblies.forEach((arm) => arm.rotation.set(0, 0, 0)); levelElapsed = 0; health = MAX_ROB_HEALTH; shields = MAX_ROB_SHIELDS; energy = maximumEnergy(upgradeLevels.energyCapacity); damageInvulnerableUntil = -Infinity; gateDone = false; cellCount = 0; levelComplete = false; hasKey = false; doorOpen = !level.key; hacking = false; hackingCamera = undefined; hackingProgress = 0; securityAlertRemaining = 0; securityMiniBossReleased = false; laserLock = undefined; secondaryLaserLock = undefined; laserChargeStarted = undefined; saberCombo = 0; lastSaberAttack = -Infinity; saberAnimation = undefined; gamepadLaserHeld = false; baseFlipperStarted = undefined;
    releaseAllInput(); keyObject.visible = Boolean(level.key); if (level.key) keyObject.position.set(level.key[0], .08, level.key[1]);
  ...
  const collision = (p, heading = robot.rotation.y) => { const { right, length } = robotAxes(heading), extentX = ROBOT_HALF_WIDTH * Math.abs(right.x) + ROBOT_HALF_LENGTH * Math.abs(length.x), extentZ = ROBOT_HALF_WIDTH * Math.abs(right.z) + ROBOT_HALF_LENGTH * Math.abs(length.z), door = levels[levelIndex].door, doorBox = door && { x: door[0], z: door[1], w: door[2] / 2, d: door[3] / 2 }; return Math.abs(p.x) + extentX > ARENA_HALF_WIDTH - ARENA_CLEARANCE || Math.abs(p.z) + extentZ > ARENA_HALF_DEPTH - ARENA_CLEARANCE || obstacles.some((o) => robotHitsBox(p, heading, o)) || (!doorOpen && doorBox && robotHitsBox(p, heading, doorBox)) || enemies.some((enemy) => enemy.userData.alive && robotHitsCircle(p, heading, enemy.position, enemyRadius(enemy))); };
  ...
  const releaseSecurityMiniBoss = (securityCamera) => {
    ...
    enemy.position.copy(spawn); enemy.userData.origin = spawn.clone(); enemy.userData.patrolPhase = elapsed * .37; enemy.userData.nextAttack = elapsed + 1.8; enemy.userData.nextSkitterSound = elapsed + .5; enemy.userData.lungeUntil = 0; securityMiniBossReleased = true;
  ...
  const activateBaseFlipper = () => {
    if (!running || complete || levelComplete || baseFlipperPresentation(elapsed - baseFlipperStarted).active) return;
    ...
    releaseAllInput(); energy -= BASE_FLIPPER_ENERGY_COST; baseFlipperStarted = elapsed; say('Base lift cycle: treads neutral, flipper deploying, chassis rising, then guarded retract.');
  };
  const tick = (dt) => {
    readInput(); const flipperActive = baseFlipperPresentation(elapsed - baseFlipperStarted).active; if (flipperActive) controls.left = controls.right = 0; if (!running || complete || levelComplete) return;
    ...
    if (!flipperActive) energy = updateDriveEnergy({ energy, maximum: maximumEnergy(upgradeLevels.energyCapacity), moving: treadsPowered, delta: dt, capacityLevel: upgradeLevels.energyCapacity });
  ...
  const animate = () => {
    ...
    const flipperPose = baseFlipperPresentation(elapsed - baseFlipperStarted); robotRig.baseFlipper.rotation.x = flipperPose.angle; robot.position.y = flipperPose.lift; robot.rotation.x = flipperPose.pitch; ui.flipperButtons.forEach((button) => { button.disabled = flipperPose.active; button.textContent = flipperPose.active ? `Base Lift ${Math.round(flipperPose.phase * 100)}%` : button.classList.contains('rob-sim__fire') ? 'BASE LIFT' : 'Base Lift · F'; }); if (baseFlipperStarted !== undefined && !flipperPose.active) baseFlipperStarted = undefined;
    ...
    if (keyObject.visible) { keyObject.rotation.y += dt * 1.7; keyObject.position.y = .1 + Math.sin(elapsed * 3.2) * .09; keyBeaconMaterial.opacity = .2 + (Math.sin(elapsed * 4.4) + 1) * .08; }
  ...
  addEventListener('keydown', (e) => { if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'KeyW', 'KeyA', 'KeyS', 'KeyD', 'Space', 'KeyQ', 'KeyF'].includes(e.code)) e.preventDefault(); const firstPress = !keys.has(e.code); keys.add(e.code); if (firstPress && e.code === 'Space') saberSlash(); if (firstPress && e.code === 'KeyQ') beginLaserCharge(); if (firstPress && e.code === 'KeyF') activateBaseFlipper(); }); addEventListener('keyup', (e) => { keys.delete(e.code); if (e.code === 'KeyQ') releaseLaserCharge(); }); addEventListener('blur', releaseAllInput);
  ...
  ui.flipperButtons.forEach((button) => button.addEventListener('click', activateBaseFlipper));
