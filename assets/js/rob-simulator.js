...
if (root) {
  ...
  const collision = (p, heading = robot.rotation.y, start = robot.position) => {
    ...
    const blockedByLedge = !pointOnLedge(start) && pointOnLedge(p) && !canMountLedge({ start, end: p, flipperAngle: baseFlipperAngle, approachEdgeZ: LEDGE.approachEdgeZ });
  ...
  const commandBaseFlipper = (target) => {
    ...
    say(target === 'forward' ? 'Rear-shaft flipper moving forward. Keep it forward and drive into the orange ledge lip.' : 'Flipper rotating behind ROB to stabilize the deck and level the torso.');
  };
  const tick = (dt) => {
    ...
    const flipperStep = advanceBaseFlipper({ angle: baseFlipperAngle, target: baseFlipperTarget, delta: dt }); baseFlipperAngle = flipperStep.angle;
    const level = levels[levelIndex], old = robot.position.clone(), oldHeading = robot.rotation.y, powered = energy > .05 ? 1 : 0, speedMultiplier = driveSpeedMultiplier(upgradeLevels.speedBoost), flipperPose = baseFlipperPresentation({ angle: baseFlipperAngle, target: baseFlipperTarget, onLedge: pointOnLedge(old) }), ledgeDriveScale = pointOnLedge(old) && !flipperPose.stabilized ? .42 : 1, left = controls.left * BASE_DRIVE_SPEED * speedMultiplier * powered * ledgeDriveScale, right = controls.right * BASE_DRIVE_SPEED * speedMultiplier * powered * ledgeDriveScale, linear = (left + right) / 2, yaw = (right - left) / 1.55 * dt, targetHeading = oldHeading + yaw;
    ...
    if (newSurfaceHeight > old.y) say('ROB mounted the ledge. Rotate the flipper backward to stabilize the rear and level the torso before full-speed travel.');
    else if (motion.collided && Math.abs(linear) > .1) say(!pointOnLedge(old) && !pointOnLedge(robot.position) && intended.z < old.z ? 'Ledge too high for the treads. Rotate the rear flipper forward, then keep driving into the orange lip.' : robot.position.distanceTo(old) > .001 ? 'Wall assist active — ROB is sliding along the open edge.' : 'Wall contact — reverse or pivot away; ROB will release cleanly.');
    if (hackingCamera) {
  ...
  addEventListener('keydown', (e) => { if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'KeyW', 'KeyA', 'KeyS', 'KeyD', 'Space', 'KeyQ', 'KeyF', 'KeyB'].includes(e.code)) e.preventDefault(); const firstPress = !keys.has(e.code); keys.add(e.code); if (firstPress && e.code === 'Space') saberSlash(); if (firstPress && e.code === 'KeyQ') beginLaserCharge(); if (firstPress && e.code === 'KeyF') commandBaseFlipper('forward'); if (firstPress && e.code === 'KeyB') commandBaseFlipper('rear'); }); addEventListener('keyup', (e) => { keys.delete(e.code); if (e.code === 'KeyQ') releaseLaserCharge(); }); addEventListener('blur', releaseAllInput);
