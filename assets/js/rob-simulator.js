...
if (root) {
  ...
  const tick = (dt) => {
    ...
    for (let i = 0; i < enemies.length; i += 1) {
      ...
      if (robotHitsCircle(robot.position, robot.rotation.y, enemy.position, (isSpider ? .78 : .7) * (enemy.userData.combatScale || 1))) { if (isSpider) playSpiderSound('impact'); else playDalekSentry(); if (damageROB(isSpider ? `${enemy.userData.name} lunge` : `${enemy.userData.name} collision`, enemy.userData.contactDamage)) return; }
    }
