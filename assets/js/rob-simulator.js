...
import { createTacticalGame } from './rob-tactical-game.mjs';
...
if (root) {
  ...
  let tacticalPadKeys = [false, false, false];
  ...
  const tactical = createTacticalGame({ root, scene, robot, rig: robotRig, arms: armAssemblies, enemies,
    state: () => ({ running: running && !complete && !levelComplete, energy, elapsed, upgrades: upgradeLevels,
      handsBusy: hacking || Boolean(hackingCamera) || pickupLeanAmount > .02 || pickupLeanRequested || pickupTask.phase === 'carrying' || Boolean(saberAnimation) }),
    energyChanged: value => { energy = value; }, shotFired: () => { lastShot = elapsed; },
    damageEnemy: (...args) => damageEnemy(...args), reward: (...args) => awardMissionPoints(...args),
    openDoor: () => { hasKey = true; keyObject.visible = false; doorOpen = true; hacking = false; hackingProgress = 0; doorObject.visible = hackTerminal.visible = false; },
    cancelCharge: () => cancelLaserCharge(), blockers: () => projectileBlockers(), say: message => say(message),
  });
  ...
  const loadLevel = (index) => {
    ...
    const relayPoint = levels[index].cells[0];
    tactical.reset(new THREE.Vector3(relayPoint[0], surfaceHeight({ x: relayPoint[0], z: relayPoint[1] }) + .846 * 2.15, relayPoint[1]));
  ...
  const beginLaserCharge = () => {
    if (tactical.press()) { laserChargeStarted = undefined; return; }
  ...
  const releaseLaserCharge = () => { tactical.release(); if (laserChargeStarted === undefined) return; const charge = laserChargeAmount(); laserChargeStarted = undefined; fireLaser(charge); };
  const cancelLaserCharge = () => { tactical.release(); laserChargeStarted = undefined; ui.laserButtons.forEach((button) => button.classList.remove('is-charging')); };
  ...
  const tick = (dt) => {
    ...
    if (!flipperStep.active && !wasFlying && supportMotion.phase === 'grounded') energy = updateDriveEnergy({ energy, maximum: maximumEnergy(upgradeLevels.energyCapacity), moving: treadsPowered, delta: dt, capacityLevel: upgradeLevels.energyCapacity, charging: laserChargeStarted !== undefined || tactical.active, secondsSinceShot: elapsed - lastShot });
    ...
    const detectingCamera = securityCameras.find((securityCamera) => !tactical.isJammed(securityCamera) && securityCameraSees({ camera: securityCamera, robot: robotPoint, elapsed, blockers: cameraBlockers, shadows: shadowZones }));
    ...
    securityCameras.forEach((securityCamera) => { const heading = cameraHeading(securityCamera, elapsed), isHackTarget = hackingCamera === securityCamera; securityCamera.group.rotation.y = heading; securityCamera.beam.visible = !securityCamera.disabled && !tactical.isJammed(securityCamera); if (!securityCamera.disabled) updateSecurityCameraBeam(securityCamera, heading, cameraBlockers); const lensColor = securityCamera.disabled ? 0x55ff95 : isHackTarget ? 0xffcf33 : 0xff243f, lensEmissive = securityCamera.disabled ? 0x0b7a38 : isHackTarget ? 0xb37700 : 0xa60018; securityCamera.lens.material.color.setHex(lensColor); securityCamera.lens.material.emissive.setHex(lensEmissive); securityCamera.lens.material.emissiveIntensity = securityCamera.disabled ? 1.25 : isHackTarget ? 2.6 + Math.sin(elapsed * 9) * .6 : securityAlertRemaining > 0 ? 3.2 : 1.8; });
    if (hacking) {
    ...
    for (let i = 0; i < enemies.length; i += 1) {
      ...
      if (tactical.isJammed(enemy.position, enemy.userData)) { enemy.userData.lungeUntil = 0; enemy.userData.nextAttack = Math.max(enemy.userData.nextAttack, elapsed + .5); continue; }
  ...
  const releaseAllInput = () => { tactical.releaseInput(); keys.clear(); [...activeDrivePointers.keys()].forEach(releaseDrive); [...activeTreadPointers.keys()].forEach(releaseTread); root.querySelectorAll('[data-tread-stick]').forEach((stick) => resetTreadStick(stick, stick.dataset.treadStick)); touch.forward = touch.steering = controls.left = controls.right = 0; gamepadLaserHeld = false; gamepadShieldHeld = false; gamepadLeanHeld = gamepadGrabHeld = false; rocketHeld = gamepadRocketHeld = false; shieldTimeRemaining = 0; cancelLaserCharge(); };
  addEventListener('keydown', (e) => { if (e.target.closest?.('input, select, textarea, [contenteditable=true]')) return; if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'KeyW', 'KeyA', 'KeyS', 'KeyD', 'Space', 'KeyQ', 'KeyF', 'KeyB', 'KeyE', 'KeyR', 'KeyC', 'KeyG', 'KeyJ', 'KeyT', 'KeyV'].includes(e.code)) e.preventDefault(); const firstPress = !keys.has(e.code); keys.add(e.code); if (firstPress && e.code === 'KeyJ') tactical.toggleJammer(); if (firstPress && e.code === 'KeyT') { cancelLaserCharge(); tactical.toggleGel(); } if (firstPress && e.code === 'KeyV') tactical.cyclePEQ(); if (firstPress && e.code === 'KeyC') togglePickupLean(); if (firstPress && e.code === 'KeyG') interactCargo(); if (firstPress && e.code === 'KeyR') setRocketHeld(true); if (firstPress && e.code === 'KeyE') activateShield(); if (firstPress && e.code === 'Space') saberSlash(); if (firstPress && e.code === 'KeyQ') beginLaserCharge(); if (firstPress && e.code === 'KeyF') commandBaseFlipper('forward'); if (firstPress && e.code === 'KeyB') commandBaseFlipper('rear'); }); addEventListener('keyup', (e) => { keys.delete(e.code); if (e.code === 'KeyR') setRocketHeld(false); if (e.code === 'KeyQ') releaseLaserCharge(); }); addEventListener('blur', releaseAllInput);
