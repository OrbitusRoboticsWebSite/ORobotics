...
export function createCurbWorkshop({container,scene,rig,onEnter}) {
  ...
  container.innerHTML='<div class="curb-heading"><div><p class="gesture-eyebrow">Guarded gesture / research preview</p><h2>A curb climb, one confirmed step at a time</h2><p>Rehearse tread advance, brake hold, flipper transfer and leveling. The base path is illustrative; this is not a tested climbing strategy.</p></div><span class="gesture-badge">Not authorized for hardware</span></div><div class="curb-layout"><div><div class="gesture-buttons"><button type="button" data-curb-run>Run rehearsal</button><button type="button" data-curb-stop>Abort</button><button type="button" data-curb-reset>Reset rehearsal</button></div><label><input type="checkbox" data-curb-auto checked> Simulate successful sensor confirmations</label><label>Inject a simulated fault<select data-curb-fault><option value="">No fault</option><option value="vision">Camera loses pose</option><option value="rollback">Rollback detected</option><option value="brake">Brake fails to hold</option><option value="obstacle">Path becomes blocked</option><option value="operator">Operator releases control</option></select></label><button type="button" data-curb-confirm>Confirm this simulation checkpoint</button><p class="curb-state" role="status" data-curb-state>Ready. No physical robot is connected.</p><ol data-curb-steps></ol><p class="gesture-note">An abort freezes this rehearsal and blocks the next step. On ROB, recovery must use the tested stop/hold response; a brake command alone cannot prove that rollback is prevented.</p></div><div><label>Sequence JSON<textarea data-curb-script spellcheck="false" aria-label="Curb sequence JSON"></textarea></label><div class="gesture-buttons"><button type="button" data-curb-apply>Validate sequence</button><button type="button" data-curb-download>Download sequence</button><button type="button" data-curb-log>Download rehearsal log</button></div><p>Angles, distances and durations are simulation examples. Physical deployment also needs a supported curb envelope, observed contact and pitch, verified braking, fresh joint references, cable limits, clearance, a dead-man control and an approved recovery procedure.</p></div></div>';
  ...
  platform.position.set(1.09,0,script.platformHeightM/2);platform.visible=false;scene.add(platform);
  const edge=new THREE.Mesh(new THREE.BoxGeometry(.018,1.22,.018),new THREE.MeshBasicMaterial({color:0xf3be67}));
  edge.position.set(.34,0,script.platformHeightM+.009);edge.visible=false;scene.add(edge);
  function signals(){
  ...
  function show(){
    const s=runner.sample();if(active){rig.pose(s.offsets);rig.rollTreads(s.world.forward);rig.root.position.set(s.world.forward-.35,0,s.world.height);rig.root.rotation.y=s.world.pitch*Math.PI/180;rig.root.updateMatrixWorld(true);}
  ...
  $('log').addEventListener('click',()=>save({schemaVersion:1,simulationOnly:true,hardwareAuthorized:false,syntheticObservations:true,profileSHA256:rig.provenance.profileSHA256,sequence:script,phase:runner.phase,fault:runner.fault,events:runner.events},'rob-curb-rehearsal-log.json'));
  rows();show();
