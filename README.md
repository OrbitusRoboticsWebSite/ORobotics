...
`/robot-lab/` is a static 50-mission Circuit Quest built from `content/robot-lab.md`, `layouts/robot-lab/single.html`, `assets/js/robot-lab.js`, and the data-driven ROB campaign in `assets/js/robot-lab-rob-missions.mjs`. It progresses from closed DC loops, solar storage, Ohm's law, AC/RLC fields, Arduino, 555 timers, and op amps into H-bridge tread power, differential drive, encoders, battery telemetry, bounded serial framing, watchdogs, Cerebro authority, separate camera media, measured arm control, and a final CerebroVisionPro session. The game uses no backend, accounts, analytics, physical robot connection, or off-device learner data and remains keyboard accessible.
