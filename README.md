...
`/learn/` is the campaign landing page for the full Orbitus learning journey, and `/books/` is the Building R.O.B. catalog. The catalog reads planned prices and future Apple Books URLs from `data/books.yaml`; empty URLs intentionally render a prepublication label instead of a purchase button. Three finished-size 36×60-inch campaign posters and their screen previews live under `static/downloads/posters/` and `static/images/posters/`. Their QR codes use the stable `/learn/`, `/rob-training-apps/`, and `/books/` routes, so update the landing pages without reprinting the posters.

`/robot-lab/` is an 80-mission Circuit Quest built from `content/robot-lab.md`, `layouts/robot-lab/single.html`, `assets/js/robot-lab.js`, and the data-driven campaigns in `assets/js/robot-lab-rob-missions.mjs` and `assets/js/robot-lab-field-missions.mjs`. It progresses from closed DC loops, solar storage, Ohm's law, AC/RLC fields, Arduino, 555 timers, and op amps into H-bridge tread power, 12/24/48 V distribution, stepper torso control, USB 3 slip-ring sensing, RPLIDAR, Orbi Ethernet, Insta360 video, Ubuntu USB-to-CAN arm control, watchdogs, Cerebro authority, and CerebroVisionPro. Completed mission ranges assemble visible tread, torso, camera/network, compute, arm, and commissioned systems in the Droid Builder. Learners can choose an allowlisted finish, housing material, and housing style, then move the same checked Droid Code into ROB Training on the web, iPhone, iPad, or Apple Vision Pro. Progress is first stored on-device; optional Apple sign-in merges a minimal learner passport into the user's private CloudKit database and enables a completion QR claim. The page has no analytics or physical robot connection and remains keyboard accessible. See `docs/robot-school-cloudkit.md` for deployment and privacy details.

`/rob-simulator/` is a locally bundled Three.js tank-driving game. Its fifteen-level campaign uses expanded arenas and models ROB with animated triangular tri-wheel tread pods, dual seven-joint AMBER-style arms, wide alternating saber swings and a third-hit torso spin, a scanning pan-and-tilt virtual gatling with a red lock indicator and blue training projectiles, ROBController-style dual tread joysticks on mobile, keyboard tread mixing, an oriented tread-and-chassis collision footprint, animated directional conveyors, Flipper Zero door and security-camera hacks, security cameras and shadow cover, battle-funded upgrade bays between levels, shared tread-and-laser system energy, reinforced campaign bosses, multiple attacking enemies, collectible energy cells, and a final docking objective. The simulator shares its local Droid Builder profile with Circuit Quest and accepts the same portable Droid Code as the native games. It supports keyboard, pointer/touch, and gamepad input and never connects to a physical robot. Completed campaigns keep a local top ten and, when CloudKit is configured and the player signs in with Apple, publish one public best score per account.
...
The basic targeting computer uses manual forward aim with a 0.8-second firing cycle and 1.8-second full charge. Its 1,200-point upgrade enables automatic locks on every laser, two independent Twin Blaster locks, a 0.25-second firing cycle, and 1.25-second full charge. Shots spend 4–12 energy (Gatling), 5–14 (Twin Blaster volley), or 8–22 (Arc Cannon), even when they miss. Charging pauses passive recovery, which resumes 1.5 seconds after the last shot.
